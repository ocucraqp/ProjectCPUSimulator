...
void ADD(Cpub *cpub) {
	...
	argB = JudgeArgB(cpub);
	result = *argA + *argB;
	SetFlag(cpub, *argA, result, 2, 3, 3, 3);
	if (cpub->cf) {
		result -= 0x100;
	}
	*argA = result;
...
void ADC(Cpub *cpub) {
	...
	SetFlag(cpub, *argA, result, 3, 3, 3, 3);
	if (cpub->cf) {
		result -= 0x100;
	}
	*argA = result;
...
void AND(Cpub *cpub) {
	...
	argB = JudgeArgB(cpub);
	result = *argA & *argB;
	SetFlag(cpub, *argA, result, 2, 0, 3, 3);
	*argA = result;
}
...
void SetFlag(Cpub *cpub, Uword argA, Uword flagSource, int cfFlag, int vfFlag,
	 ...
	 * 1:	set
	 * 2:	no effect
	 * 3:	Depends on argA and flagSource
	 * */
